...
GPA_TABLE = {"A": 4.5, "B": 4.0, "C": 3.0, "D": 2.0, "F": 0.0}  # FIX: A 4.0 -> 4.5
...
def average_score(student):
    """학생의 평균."""
    return total_score(student) / 3  # FIX: len(student)(=4, 이름키 포함) -> 3
...
def subject_average(students, subject):
    ...
    total = 0
    for stu in students:
        total += stu[subject]  # FIX: stu["국어"] -> stu[subject]
...
def rank_list(students):
    """총점 기준 정렬(내림차순)."""
    return sorted(students, key=lambda s: total_score(s), reverse=True)  # FIX: reverse=True
